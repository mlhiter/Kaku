...
## 13. M2 设计文档索引（2026-04-02）

1. Agent 状态总线与多 Provider 适配设计稿：
   - `docs/m2-agent-status-adapter-design.md`
2. 目标：通过统一事件协议 + Adapter 插件化，实现 Codex/Claude 等 provider 的可插拔状态接入，避免产品层与单一 provider 绑定。

## 14. M2-1A/B 实施进度（2026-04-02）

1. 已新增状态域模块：
   - `kaku-gui/src/agent_status/events.rs`
   - `kaku-gui/src/agent_status/manager.rs`
   - `kaku-gui/src/agent_status/adapters/{mod.rs,codex.rs,claude.rs}`
2. 已完成统一状态类型与状态机规则：
   - `SessionStatus` / `SessionStatusSource` / `SessionStatusConfidence`
   - `AgentEvent` 协议与合法跳转校验
3. 已打通最小运行链路（M2-1B）：
   - user-var 入口（`emit_user_var_event`）接入 adapter registry
   - adapter 产出 `AgentEvent` -> `SessionStatusManager`
   - 状态变化回写 sidebar session 持久化字段
4. Codex heuristic 适配已可用：
   - 识别 `kaku_last_cmd` / `kaku_last_exit_code`
   - 处理重复 exit 信号去重
   - 修复“同命令二次运行被误判重复”问题（加入 command generation）
5. 兼容性：
   - `sessions.json` 新字段均 `serde(default)`，可读取旧数据
   - 旧状态字符串解析保持兼容
6. 命令验证（已通过）：
   - `cargo check -p kaku-gui`
   - `cargo test -p kaku-gui agent_status:: -- --nocapture`
   - `cargo test -p kaku-gui render::sidebar::tests:: -- --nocapture`
   - `cargo build -p kaku-gui`

## 15. M2-2 进度补充（2026-04-02）

1. `AgentAdapter` 新增 pane output 观察入口（用于非结构化运行中信号识别）。
2. `CodexAdapter` 增加低置信度审批提示识别：
   - 当尾部输出匹配审批提示（如 `Approve ... [y/N]`）时发出 `ApprovalRequired`。
   - 审批提示消失时发出 `ApprovalResolved(approved=true)` 并回到 `running`。
3. `TermWindow` 状态持久化改为 debounce 写盘：
   - 状态变更先入队
   - 300ms 合并落盘，降低频繁 `sessions.json` 原子写入开销
   - 关闭窗口前会强制 flush 一次
4. `PaneOutput` 链路接入状态总线，并增加 450ms 探测节流，仅在 codex 上下文采样，避免高频扫描影响交互。
5. 单测补充：
   - Codex 审批提示提取与 `need_approve -> running` 事件流单测已加入并通过。

## 16. M2-2 结构化信号补充（2026-04-02）

1. `CodexAdapter` 新增 App Server JSON 通知解析：
   - 支持 `turn/started`、`thread/status/changed`、`item/autoApprovalReview/*`、`turn/completed`、`error` 映射到统一 `AgentEvent`。
2. 结构化通知与文本启发式协同：
   - 若检测到 App Server JSON 通知，优先按结构化事件驱动状态。
   - 未检测到结构化通知时，保持原文本审批提示识别兜底。
3. 去重与稳态处理：
   - 增加按 pane 的通知签名去重，避免重复尾行导致状态抖动。
4. 关键限制验证结论：
   - 外部独立 `codex` / `codex exec` 进程暂无法仅靠独立 App Server 实时订阅运行态（可读取历史线程，但实时 active/approval 不稳定），因此 heuristic 主链路仍需保留。
